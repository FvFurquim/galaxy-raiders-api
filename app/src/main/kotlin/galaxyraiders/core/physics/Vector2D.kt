...
data class Vector2D(val dx: Double, val dy: Double) {
  ...
  val radiant: Double
    get() {
      val unitPoint = Point2D(this.unit.dx, this.unit.dy)
      val unitSign = sign(this.unit.dy)
      val size = unitPoint.distance(Point2D(1.0, 0.0))

      return unitSign * acos(1 - size.pow(2) / 2.0)
    }

  val degree: Double
    get() = this.radiant * (180.0 / Math.PI)
